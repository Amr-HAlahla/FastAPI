...
class ShowCommand(Command):
    ...
    def run(self, options: Values, args: List[str]) -> int:
        if not args:
            logger.warning('ERROR: Please provide a package user_name or names.')
            return ERROR
        query = args
...
def search_packages_info(query: List[str]) -> Iterator[_PackageInfo]:
    """
    Gather details from installed distributions. Print distribution user_name,
    ...
    def _get_requiring_packages(current_dist: BaseDistribution) -> List[str]:
        return [
            ...
            if current_dist.canonical_name in {
                canonicalize_name(d.user_name) for d in dist.iter_dependencies()
            }
        ]
    ...
    for query_name in query_names:
        ...
        yield _PackageInfo(
            ...
            requires=[req.user_name for req in dist.iter_dependencies()],
